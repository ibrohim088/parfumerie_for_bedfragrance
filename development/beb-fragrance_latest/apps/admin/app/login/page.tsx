...
async function apiVerifyOtp(phone: string, otp: string): Promise<{ accessToken: string; refreshToken: string; user: any }> {
  ...
  if (!res.ok) {
    if (data.code === 'USER_NOT_FOUND') {
      throw new Error(
        "Bu raqam bilan ro'yxatdan o'tilmagan. Avval Telegram botda ro'yxatdan o'ting, " +
        "so'ngra shu akkauntga admin huquqini bering."
      );
    }
    throw new Error(data.message ?? 'OTP tekshirishda xato');
  }
  return data.data;
}
